!(function ($, window, document, undefined) {
  ...
  var Plugin = function (elem, options) {
    this.$elem = elem;
    this.$btn = $('#page1_button');
    this.$oMask = $('#mask_shadow');
    this.$oTitle = this.$elem.find('.pop_title');
    this.$title_text = this.$oTitle.find('.pop_title_text');
    this.$close = this.$oTitle.find('.pop_title_close');
  ...
  Plugin.prototype = {
    ...
    drag: function (ev) { // 拖拽事件
      ...
      var disY = oEvent.clientY - this.$elem.offset().top + 100;
      var _move = true;
